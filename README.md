...
```bash
# 安裝 Python 依賴
pip install datalab-python-sdk sentence-transformers
...
Reference_data/*.pdf
        │
        ▼  batch_process.py（Chandra OCR 2 高精度解析 + 300字切片）
        │  需要：pip install datalab-python-sdk
...
**所有 API Key 均透過環境變數管理，不會寫入程式碼或提交到 Git。**
...
```bash
cp .dev.vars.example .dev.vars
# 編輯 .dev.vars，填入你的 TAVILY_API_KEY
```
...
```bash
npx wrangler secret put TAVILY_API_KEY
# 輸入你的 Tavily API Key（取得：https://tavily.com）
```

#### Chandra OCR 2（知識庫建置用）

```bash
cp .env.example .env
# 編輯 .env，填入你的 DATALAB_API_KEY
# （取得：https://www.datalab.to/app/keys，新帳號享 $5 免費額度）
...
```
1. 安裝依賴          pip install datalab-python-sdk sentence-transformers
2. 設定 API Key      export DATALAB_API_KEY=your_key
3. 放入 PDF          將檔案放到 Reference_data/ 資料夾
4. 解析切片          python3 batch_process.py   → 輸出 document_chunks.json
5. 向量化            python3 upload_prep.py      → 輸出 vectorize_upload.ndjson
6. 上傳向量庫        npx wrangler vectorize insert medical-index --file=vectorize_upload.ndjson
```

### 步驟說明

**步驟 4 — `batch_process.py`**

透過 **Chandra OCR 2**（Datalab API）以 `accurate` 高精度模式解析 PDF，
每份文件切成 300 字區塊並輸出 `document_chunks.json`。
解析時會顯示每份文件的品質分數（0–5 分）。

**步驟 5 — `upload_prep.py`**

使用本地 `BAAI/bge-base-en-v1.5` 模型（與 Cloudflare Vectorize 相同模型）
將文字區塊批次轉換成向量，輸出 `vectorize_upload.ndjson`。
